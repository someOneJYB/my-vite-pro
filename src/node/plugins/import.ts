...
import { cleanUrl, getShortName, isInternalRequest, isJSType } from '../utils'

export function importAnalysisPlugin(): Plugin {
  let serverContext: ServerContext
  return {
    name: 'my-vite:import-analysis',
    setServer(s) {
      serverContext = s
    },
    async transform(code: string, id: string) {
      if (!isJSType(id) || isInternalRequest(id)) {
        return null
      }
      await init
      ...
      moduleGraph.updateModule(curMod, importedModules)
